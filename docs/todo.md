...
- [ ] Tab bar icon weight mismatch. Streams uses `UIImage(systemName: "radio")`, an SF Symbol whose dotted speaker grille reads much heavier at 25pt than the four thin-line assets beside it (`podcasts_tab`, `playlists_tab`, `discover_tab`, `profile_tab`). Pre-existing — predates M12, which passes the symbol through unchanged. Settle the whole set at M12.1: that milestone adds an Overflow "More" icon, and a promoted playlist draws its icon from `EpisodeFilter.iconImageName()`, which is sized for list rows rather than tab items and will likely read the same way. Options: give Streams a custom thin-line asset, or move all five to SF Symbols. **Resolved by observation at M12.1 (2026-09-11), not yet implemented.** With a layout of `[playlist:new-releases, streams]` rendered on the simulator, the Overflow `ellipsis` and the playlist clock (from `EpisodeFilter.iconImageName()`) both read as thin outlines consistent with the existing four assets. `radio` is the sole outlier, so the cheap fix wins: add one custom thin-line radio asset and leave the other six alone. No wholesale move to SF Symbols.
